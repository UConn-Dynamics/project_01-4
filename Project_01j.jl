...
begin
	@variables x1(t) x2(t)
    @parameters c   # damping coefficient

    # original RHS from Euler–Lagrange
    original_rhs = substitute(sol, Dict(θ => x1, θ_dot => x2))

    # add viscous damping: τ_damp = -c * θ̇
    # EoM: θ̈ = original_rhs - (c/(m*L^2))*θ̇
    damped_rhs = original_rhs - (c/(m*L^2)) * x2
...
begin
    ...
    p_vals = Dict(L => 0.15, g => 9.8, Ω => 2.0, m => 0.1, w1 => 0.1, h1 => 0.2, c => 0.002)     # damping coefficient
...
# ╔═╡ a64ae2a4-e481-402b-8ea8-c36de5620973
begin
	begin
	    # Faster swinging initial conditions (bigger angular velocity)
	    initial_angle_fast_swing = 0.6          # radians 
	    initial_speed_fast_swing = 6.0          # rad/s (makes the bob swing faster)
	
	    # Initial state vector for faster swinging solution
	    u0_fast_swing = [
	        x1 => initial_angle_fast_swing,
	        x2 => initial_speed_fast_swing
	    ]
	
	    # Use the SAME rotation speed Ω as your slow case (same pole rotation)
	    p_fast_swing = Dict(
	        L => 0.15,
	        g => 9.8,
	        Ω => p_slow[Ω],   # << SAME ROTATION SPEED
	        m => 0.1,
	        w1 => 0.1,
	        h1 => 0.2,
	        c => 0.00001         
	    ...
	    # Solve time interval
	    time_span_fast_swing = (0.0, 12.0)
	
	    # Build and solve the ODE
	    problem_fast_swing = ODEProblem(
	        sys,
	        merge(Dict(u0_fast_swing), p_fast_swing),
	        time_span_fast_swing
	    )
	
	    sol_fast_swing = solve(
	        problem_fast_swing,
	        Rodas5();
	        reltol = 1e-6,
	        abstol = 1e-8,
...
# ╔═╡ e393d30a-eac2-4658-9113-319747ae36ca
begin
    # Extract θ(t) and time
    theta_fast_swing = sol_fast_swing[x1]
    time_fast_swing  = sol_fast_swing.t

    # Compute rotation angle φ(t) using SAME Ω
    phi_fast_swing = p_fast_swing[Ω] .* time_fast_swing

    # Read physical parameters for readability
    pendulum_length = p_fast_swing[L]
    frame_width     = p_fast_swing[w1]
    frame_height    = p_fast_swing[h1]

    # Compute pendulum bob position in 3D
    bob_x_fast = frame_width .* cos.(phi_fast_swing) .+
                 pendulum_length .* sin.(theta_fast_swing) .* cos.(phi_fast_swing)

    bob_y_fast = frame_width .* sin.(phi_fast_swing) .+
                 pendulum_length .* sin.(theta_fast_swing) .* sin.(phi_fast_swing)

    bob_z_fast = frame_height .-
                 pendulum_length .* cos.(theta_fast_swing)

    # Pivot point (top of pendulum rod)
    pivot_x_fast = frame_width .* cos.(phi_fast_swing)
    pivot_y_fast = frame_width .* sin.(phi_fast_swing)
    pivot_z_fast = fill(frame_height, length(time_fast_swing))
end

# ╔═╡ f266ff36-94ca-4a79-9fd1-34d37f5a6ade
begin
    max_radius_fast = frame_width + pendulum_length

    fast_swing_animation = @animate for i in eachindex(time_fast_swing)

        # Trace of bob path
        plot3d(
            bob_x_fast[1:i], bob_y_fast[1:i], bob_z_fast[1:i];
            label="Trace", linecolor=:blue, linestyle=:dot, linewidth=2,
            xlim=(-max_radius_fast, max_radius_fast),
            ylim=(-max_radius_fast, max_radius_fast),
            zlim=(0, frame_height + pendulum_length + 0.1),
            xlabel="x", ylabel="y", zlabel="z",
            title="Fast Swinging Damped Pendulum (t = $(round(time_fast_swing[i], digits=2)) s)",
            aspect_ratio=:equal
        )

        # Pendulum rod
        plot3d!(
            [pivot_x_fast[i], bob_x_fast[i]],
            [pivot_y_fast[i], bob_y_fast[i]],
            [pivot_z_fast[i], bob_z_fast[i]];
            label="Rod", linecolor=:green, linewidth=3
        )

        # Bob
        scatter3d!(
            [bob_x_fast[i]], [bob_y_fast[i]], [bob_z_fast[i]];
            label="Bob", markercolor=:red, markersize=6
        )

        # Fixed vertical support
        plot3d!([0,0], [0,0], [0,frame_height];
            label="Frame", linecolor=:black, linewidth=3
        )
        ...
        plot3d!(
            [0, pivot_x_fast[i]],
            [0, pivot_y_fast[i]],
            [frame_height, pivot_z_fast[i]];
            label="Arm", linecolor=:black, linewidth=3
        )
    end
end

# ╔═╡ 36d683c2-0415-4ef4-bf75-ad7f66f5eeaf
gif(fast_swing_animation, "pendulum_fast_swing.gif", fps=30)
