...
# ╔═╡ 8b060abe-42fc-4e47-87b4-0b3af3fc7314
begin
...
md"""
# Project 01 - Spinning Pendulum and the Lagrange Equations

This notebook models a pendulum attached to a rotating frame. The goals are to:

- derive the equation of motion using the Lagrangian \(L = T - V\)
- solve the motion for a slow and a fast rotation speed
- visualize the motion with plots and animations
"""

# ╔═╡ d86c932d-25cb-4b05-9d2e-58a2338353f5
md"""
## Imported packages

This notebook uses these packages:

- `OrdinaryDiffEq` for numerical ODE solving
- `Plots` for time plots, phase plots, 3D plots, and GIF animation
- `PlutoUI` for displaying saved GIF files inside Pluto

These packages are imported once below and then used throughout the notebook.
"""

# ╔═╡ 6d2b5427-f122-4026-a43e-44b6f18b209f
md"""
## Geometry and model assumptions

The frame dimensions and pendulum properties are taken from the project statement.

Assumptions used in the model:

- the rod is massless
- the bob is a point mass
- the frame rotates at constant angular speed \(\Omega\)
- the pendulum swings only in the rotating \(x'-z'\) plane
- damping is neglected
"""

# ╔═╡ 223d2f0a-2c3a-4197-8c68-3f2fd79d89ef
begin
	frame_height = 0.20
	frame_offset = 0.10
...
# ╔═╡ 75ea7347-9a7c-4fa1-b245-5caa0052c1c7
md"""
## Initial conditions and simulation settings

This block defines the initial angle, initial angular velocity, simulation time span,
and the two rotation speeds used for comparison.

Package reference: no new package functions are used here.
"""

# ╔═╡ 230db7da-403c-4cf7-b938-4bf1a3ccc460
begin
	initial_angle = deg2rad(20.0)
	initial_angular_velocity = 0.0
	simulation_time_span = (0.0, 10.0)
	slow_rotation_speed = 2.0
	fast_rotation_speed = 12.0
end

# ╔═╡ 75279926-05b1-42bf-b134-0dd9b6513053
md"""
## Position of the pendulum bob

Using the generalized coordinate \(\theta(t)\), the bob position in the rotating frame is

\[
x' = w_1 + L\sin\theta,\qquad y' = 0,\qquad z' = h_1 - L\cos\theta
\]

In the inertial frame, the rotating frame angle is \(\phi = \Omega t\), so

\[
x = (w_1 + L\sin\theta)\cos(\Omega t)
\]
\[
y = (w_1 + L\sin\theta)\sin(\Omega t)
\]
\[
z = h_1 - L\cos\theta
\]
...
# ╔═╡ 6265d139-e11d-4858-b4e2-159ff1ed7300
md"""
...
\[
T = \frac12 m\left[L^2\dot{\theta}^2 + \Omega^2(w_1 + L\sin\theta)^2\right]
\]
...
gives the nonlinear equation of motion

\[
\ddot{\theta}
=
\frac{\Omega^2}{L}(w_1 + L\sin\theta)\cos\theta
-
\frac{g}{L}\sin\theta
\]

This is the ODE solved numerically below.
"""

# ╔═╡ f2fb86d3-e501-4e90-a16f-20188771567f
md"""
## Equation of motion function

This code defines the first-order ODE system equivalent to the pendulum equation of motion.

Package reference:
- `OrdinaryDiffEq` will use this function inside `ODEProblem`
"""

# ╔═╡ 9788c96a-ad5f-478a-85fe-5bcc293b2ddd
function spinning_pendulum_equations!(state_derivative, state, parameters, time)
	angle, angular_velocity = state
	rotation_speed, pendulum_length, frame_offset, gravity = parameters

	state_derivative[1] = angular_velocity
	state_derivative[2] =
		(rotation_speed^2 / pendulum_length) *
		(frame_offset + pendulum_length * sin(angle)) *
		cos(angle) -
		(gravity / pendulum_length) * sin(angle)
end

# ╔═╡ e93b42d5-cc2a-468d-a654-05282513c1ca
md"""
## Initial state vector

This block stores the initial angle and angular velocity in a vector form for the ODE solver.

Package reference: no new package functions are used here.
"""

# ╔═╡ 46b678af-bd8d-449f-a05e-b25d72d33231
initial_state = [initial_angle, initial_angular_velocity]

# ╔═╡ b17fab4d-cba7-43c6-a398-6ccc54dadb46
md"""
## Slow-rotation ODE problem

This block creates the slow-case ODE problem.

Package reference:
- `OrdinaryDiffEq.ODEProblem`
"""

# ╔═╡ 819021e5-01dd-49eb-94fc-4449ff674f1d
slow_problem = ODEProblem(
	spinning_pendulum_equations!,
	initial_state,
	simulation_time_span,
	(slow_rotation_speed, pendulum_length, frame_offset, gravitational_acceleration)
)

# ╔═╡ 597b7e1c-1f5f-4e88-9272-fdac6f39b413
md"""
## Fast-rotation ODE problem

This block creates the fast-case ODE problem.

Package reference:
- `OrdinaryDiffEq.ODEProblem`
"""

# ╔═╡ 269844b6-b683-4dc3-9db3-5c0e7a41db79
fast_problem = ODEProblem(
	spinning_pendulum_equations!,
	initial_state,
	simulation_time_span,
	(fast_rotation_speed, pendulum_length, frame_offset, gravitational_acceleration)
)

# ╔═╡ 073dbf91-1a9a-4d22-82b4-33180ea18961
md"""
## Solve the slow case

This block numerically solves the slow-rotation problem.
...
# ╔═╡ cb512ba6-eb98-4bda-b98b-bada983ee666
slow_solution = solve(
	slow_problem,
	Tsit5(),
	abstol = 1e-9,
...
# ╔═╡ 854876f0-06d6-4b24-867d-fc1e568727da
md"""
## Solve the fast case

This block numerically solves the fast-rotation problem.
...
# ╔═╡ d2cc24f4-077e-4567-b8b3-9697369088b1
fast_solution = solve(
	fast_problem,
	Tsit5(),
	abstol = 1e-9,
...
# ╔═╡ 633684f6-7cbc-4a64-916f-17f8e2e5f91f
md"""
## Angle versus time plot

This plot compares the pendulum angle over time for the slow and fast rotation speeds.
...
# ╔═╡ b092dbcf-a754-4091-ad4c-4a79b7c9c846
angle_vs_time_plot = plot(
	slow_solution.t,
	rad2deg.(slow_solution[1, :]),
	label = "slow rotation",
	xlabel = "time (s)",
	ylabel = "angle (deg)",
	title = "Pendulum angle versus time",
	lw = 2
)

# ╔═╡ 6cdef11e-3a38-48af-bd61-30ee60fbc348
plot!(
	angle_vs_time_plot,
	fast_solution.t,
	rad2deg.(fast_solution[1, :]),
	label = "fast rotation",
	lw = 2
)

# ╔═╡ 2c8b9122-5280-444a-9dd6-28ddc002d816
md"""
## Angular velocity versus time plot

This plot compares angular velocity for the slow and fast cases.
...
# ╔═╡ 00fdd48a-a0e2-489f-95af-89fb468e8e0d
angular_velocity_plot = plot(
	slow_solution.t,
	slow_solution[2, :],
	label = "slow rotation",
	xlabel = "time (s)",
...
# ╔═╡ 74c00fdf-5589-466f-ab5f-d37a41d616f8
plot!(
...
# ╔═╡ 287a30fe-3f6a-481e-848a-533edb4cb458
md"""
## Phase portrait

This plot shows angular velocity versus angle for both solutions.
...
# ╔═╡ a4127e5e-bf82-4a8d-9c31-50a6b5a0e39b
phase_portrait_plot = plot(
	rad2deg.(slow_solution[1, :]),
	slow_solution[2, :],
	label = "slow rotation",
	xlabel = "angle (deg)",
...
# ╔═╡ 13d27d4e-b103-4c63-bfeb-a4c1e19956a4
plot!(
	phase_portrait_plot,
	rad2deg.(fast_solution[1, :]),
	fast_solution[2, :],
...
# ╔═╡ 8b05490f-1e1e-48c9-b270-0397684161ec
md"""
## Convert angular motion to 3D position

This helper function converts the solved angle history into the bob's inertial-frame
\(x\), \(y\), and \(z\) coordinates.

Package reference: no new package functions are required here.
"""

# ╔═╡ 6bace8b1-e569-48a0-837e-437859712a8a
function bob_position_coordinates(solution, rotation_speed, frame_offset, frame_height, pendulum_length)
	time_values = solution.t
	angle_values = solution[1, :]

	radial_distance = frame_offset .+ pendulum_length .* sin.(angle_values)
	position_x = radial_distance .* cos.(rotation_speed .* time_values)
	position_y = radial_distance .* sin.(rotation_speed .* time_values)
	position_z = frame_height .- pendulum_length .* cos.(angle_values)

	return position_x, position_y, position_z
end

# ╔═╡ a5f6df1e-007e-448c-b1bd-53c46c7a9d23
md"""
## 3D coordinates for the slow case

This block computes the bob trajectory for the slow-rotation solution.

Package reference: uses the helper function defined above.
"""

# ╔═╡ 1aae0097-d505-4518-aa3b-3dfcb2b7006c
slow_position_x, slow_position_y, slow_position_z =
	bob_position_coordinates(
		slow_solution,
		slow_rotation_speed,
		frame_offset,
		frame_height,
...
# ╔═╡ 113f45d7-ec46-4d6c-9e67-15d2a1e6a9cb
md"""
## 3D coordinates for the fast case

This block computes the bob trajectory for the fast-rotation solution.

Package reference: uses the helper function defined above.
"""

# ╔═╡ 148162d3-ca80-44e3-b6fe-26cb0a13a7e4
fast_position_x, fast_position_y, fast_position_z =
	bob_position_coordinates(
		fast_solution,
		fast_rotation_speed,
		frame_offset,
...
# ╔═╡ 0b11e514-1452-41e1-b5d7-2724b5d55c04
md"""
## 3D trajectory plot

This plot compares the 3D motion of the bob for both rotation speeds.
...
# ╔═╡ f2594d98-9a1e-46c2-ace0-4878ed9012da
trajectory_3d_plot = plot(
	slow_position_x,
	slow_position_y,
	slow_position_z,
	label = "slow rotation",
	...
	ylabel = "y (m)",
	zlabel = "z (m)",
	title = "3D trajectory of pendulum bob",
	lw = 2
)

# ╔═╡ 8df56955-98f8-4ddb-8c3c-cacd20066973
plot!(
	trajectory_3d_plot,
	fast_position_x,
	fast_position_y,
...
# ╔═╡ 86e54ae5-f603-4826-9d18-6d7c726fd60d
scatter!(
	trajectory_3d_plot,
	[0.0],
	[0.0],
	[frame_height],
	label = "rotation axis top"
)

# ╔═╡ 11487e7e-0f06-4698-ad0d-fcf668b35239
md"""
## Animation helper function

This function creates a GIF of the rotating frame and pendulum bob motion, then saves it to disk.
...
# ╔═╡ 973bfcb9-87eb-4e80-a52a-467f68484d17
function create_pendulum_animation(
	...
	frame_step = 3,
	file_name = "pendulum.gif"
)
	time_values = solution.t
	position_x, position_y, position_z =
		bob_position_coordinates(solution, rotation_speed, frame_offset, frame_height, pendulum_length)

	anim = @animate for index in 1:frame_step:length(time_values)
		current_angle = rotation_speed * time_values[index]
		pivot_x = frame_offset * cos(current_angle)
		pivot_y = frame_offset * sin(current_angle)
		pivot_z = frame_height
		bob_x = position_x[index]
		bob_y = position_y[index]
		bob_z = position_z[index]
		...
		current_plot = plot(
			...
			ylabel = "y (m)",
			zlabel = "z (m)",
			title = "Spinning pendulum",
			legend = false
		)

		plot!(current_plot, [0, 0], [0, 0], [0, frame_height + 0.05], lw = 2)
		plot!(current_plot, [0, pivot_x], [0, pivot_y], [frame_height, frame_height], lw = 3)
		plot!(current_plot, [pivot_x, bob_x], [pivot_y, bob_y], [pivot_z, bob_z], lw = 4)
		scatter!(current_plot, [pivot_x], [pivot_y], [pivot_z], markersize = 5)
		scatter!(current_plot, [bob_x], [bob_y], [bob_z], markersize = 7)
		plot!(current_plot, position_x[1:index], position_y[1:index], position_z[1:index], lw = 1)

		current_plot
	end

	gif(anim, file_name, fps = frames_per_second)
	return file_name
end

# ╔═╡ 1b1d970d-ed07-435f-ac41-030c2e7be9c3
md"""
## Slow-rotation animation

This block creates and displays the slow-case GIF.

Package reference:
- uses the helper function above
- `PlutoUI.LocalResource` displays the saved file
"""

# ╔═╡ 2ceecef7-e9c3-43cb-95d2-43610c370288
slow_animation_file = create_pendulum_animation(
	slow_solution,
	slow_rotation_speed,
	frame_offset = frame_offset,
	frame_height = frame_height,
	pendulum_length = pendulum_length,
	file_name = "slow_spin.gif"
)

# ╔═╡ 20013142-90af-49ca-a8ce-0e8f577640d2
LocalResource(slow_animation_file)

# ╔═╡ 461560b6-25f3-44b2-b011-eb883121860b
md"""
## Fast-rotation animation

This block creates and displays the fast-case GIF.
...
# ╔═╡ 16559da9-0f3e-46e5-b806-614c546f7ef2
fast_animation_file = create_pendulum_animation(
	fast_solution,
	fast_rotation_speed,
	frame_offset = frame_offset,
	frame_height = frame_height,
	pendulum_length = pendulum_length,
	file_name = "fast_spin.gif"
)

# ╔═╡ 76bafe53-d930-49d5-9aed-5368a496fcaa
LocalResource(fast_animation_file)

# ╔═╡ 05ac49b4-4df4-4a30-b728-701f6fb47f5f
md"""
## Discussion of results

The equation of motion was derived from the Lagrangian \(L = T - V\) and solved numerically for two rotation speeds.

The plots show that increasing the frame rotation speed changes the pendulum response noticeably because the rotational term
\[
\frac{\Omega^2}{L}(w_1 + L\sin\theta)\cos\theta
\]
becomes more important relative to gravity.

The slow and fast animations provide a direct visual comparison of how the pendulum behaves in each case.
"""
...
manifest_format = "2.0"
project_hash = "e4c7f7327792efd1a1f95a699aebb9872ce76626"
